...
@pytest.mark.asyncio
async def test_hierarchy_agent_persists_thread_across_turns():
    """Two turns on one thread accumulate history, including nested delegation."""
    ...
    turn_responses = [
        ...
        AIMessage(content="The Maths Expert says the answer is 5."),
        AIMessage(content="The result is 5."),
    ]

    from agents.langgraph_supervisor_hierarchy_agent import workflow

    agent = workflow(FakeToolModel(turn_responses + turn_responses), checkpointer=MemorySaver())
    config = {"configurable": {"thread_id": "thread-1"}}

    await agent.ainvoke({"messages": [HumanMessage(content="Add 2 and 3")]}, config)
    first_turn = await agent.aget_state(config)
    first_count = len(first_turn.values["messages"])

    await agent.ainvoke({"messages": [HumanMessage(content="And 4 and 5?")]}, config)
    second_turn = await agent.aget_state(config)

    assert len(second_turn.values["messages"]) > first_count
    assert second_turn.values["messages"][0].content == "Add 2 and 3"
