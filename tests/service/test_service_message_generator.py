...
async def test_three_layer_supervisor_hierarchy_agent_with_fake_model():
    responses = [
        AIMessage(
            content="",
            tool_calls=[
                ToolCall(name="research_expert", args={"request": "Add 2 and 3"}, id="call-1")
            ],
        ),
        AIMessage(
            content="",
            tool_calls=[ToolCall(name="math_expert", args={"request": "2+3"}, id="call-2")],
        ),
        AIMessage(
    ...
    with patch("service.service.get_agent", return_value=agent):
        ...
        assert messages[0].tool_calls[0]["name"] == "research_expert"
        assert messages[1].tool_calls[0]["name"] == "math_expert"
