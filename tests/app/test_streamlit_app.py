...
def multi_agent_messages():
    ...
    A supervisor delegates by calling a tool that runs a sub-agent, so the sub-agent's
    messages stream between the delegation tool call and that tool's result.
    """
    ...
    # Supervisor delegates to agent A
    delegate_a = ChatMessage(
        type="ai",
        content="Delegating to agent A...",
        tool_calls=[{"name": "agent_a", "id": "delegate-a", "args": {"request": "task_1"}}],
    )

    # Agent A delegates to agent B (sub-agent)
    delegate_b_from_a = ChatMessage(
        type="ai",
        content="Agent A delegating to agent B...",
        tool_calls=[{"name": "agent_b", "id": "delegate-a-b", "args": {"request": "task_2"}}],
    )
    agent_b_final = ChatMessage(type="ai", content="Agent B finished.")
    # Agent B's result repeats its final message, so the UI doesn't draw it again
    agent_b_result = ChatMessage(
        type="tool", content="Agent B finished.", tool_call_id="delegate-a-b"
    )

    agent_a_final = ChatMessage(type="ai", content="Agent A finished.")
    agent_a_result = ChatMessage(type="tool", content="result_1", tool_call_id="delegate-a")

    # Supervisor continues and delegates to agent C (sibling to A)
    supervisor_continues = ChatMessage(
        type="ai",
        content="Now delegating to agent C...",
        tool_calls=[{"name": "agent_c", "id": "delegate-c", "args": {"request": "task_3"}}],
    )
    agent_c_final = ChatMessage(type="ai", content="Agent C finished.")
    agent_c_result = ChatMessage(type="tool", content="result_3", tool_call_id="delegate-c")
    ...
    return {
        ...
        "delegate_b_from_a": delegate_b_from_a,
...
def assert_popover(popover, name, args_json, output):
    assert popover.type == "popover"
    assert popover.proto.popover.label == name
    assert popover.proto.popover.icon == "🛠️"
    assert popover.markdown[0].value == f"**Tool:** {name}"
    assert popover.markdown[1].value == "**Input:**"
    assert popover.json[0].value == args_json
    assert popover.markdown[2].value == "**Output:**"
    assert popover.markdown[3].value == output


def assert_sub_agent_status(status, name, request):
    assert status.label == f"💼 Sub Agent: {name}"
    assert status.children[0].value == "Input:"
    assert status.children[1].value == f'{{"request": "{request}"}}'
...
async def test_app_streaming_single_sub_agent(mock_agent_client, multi_agent_messages):
    ...
    assert ai_message.children[0].value == "Delegating to agent A..."

    status_agent = ai_message.status[0]
    assert status_agent == ai_message.children[1], "Second child should be the first status"
    assert_sub_agent_status(status_agent, "agent_a", "task_1")

    assert status_agent.children[2].value == "Starting tool 1..."
    assert_popover(status_agent.children[3], "do_work_1", '{"my-arg": "value"}', "Tool 1 complete")
    assert status_agent.children[4].value == "Starting tool 2..."
    assert_popover(
        status_agent.children[5], "do_work_2", '{"my-arg-2": "value"}', "Tool 2 complete"
    )
    assert status_agent.children[6].value == "Agent A finished."
    assert status_agent.children[7].value == "Output:"
    assert status_agent.children[8].value == "result_1"
...
async def test_app_streaming_sequential_sub_agents(mock_agent_client, multi_agent_messages):
    ...
    # Supervisor -> Agent A (using tool 1) -> Supervisor -> Agent C (using tool 2) -> Supervisor
    messages = multi_agent_messages
    ...
    assert_sub_agent_status(status_a, "agent_a", "task_1")
    assert status_a.children[2].value == "Starting tool 1..."
    assert_popover(status_a.children[3], "do_work_1", '{"my-arg": "value"}', "Tool 1 complete")
    assert status_a.children[5].value == "Output:"
    assert status_a.children[6].value == "result_1"

    assert ai_message.children[2].value == "Now delegating to agent C..."

    status_c = ai_message.status[1]
    assert status_c == ai_message.children[3], "Fourth child should be the second status"
    assert_sub_agent_status(status_c, "agent_c", "task_3")
    assert status_c.children[2].value == "Starting tool 2..."
    assert_popover(status_c.children[3], "do_work_2", '{"my-arg-2": "value"}', "Tool 2 complete")
    assert status_c.children[5].value == "Output:"
    assert status_c.children[6].value == "result_3"

    assert ai_message.children[4].value == "All agents have completed their tasks successfully."

    assert len(ai_message.children) == 6, (
        f"Should have 6 children: delegate to a, status for a, delegate to c, status for c, final message, feedback stars - got {len(ai_message.children)}"
    )
...
async def test_app_streaming_nested_sub_agents(mock_agent_client, multi_agent_messages):
    ...
    # Supervisor -> Agent A (using tool 1) -> Agent B (using tool 2) -> Agent A -> Supervisor
    messages = multi_agent_messages
    ...
    async def amessage_iter():
        for msg in [
            ...
            messages["tool_1"],
            messages["tool_1_result"],
            messages["delegate_b_from_a"],
            messages["tool_2"],
            messages["tool_2_result"],
            messages["agent_b_final"],
            messages["agent_b_result"],
            messages["agent_a_final"],
            messages["agent_a_result"],
            messages["supervisor_final"],
        ]:
    ...
    assert status_a.children[2].value == "Starting tool 1..."
    assert_popover(status_a.children[3], "do_work_1", '{"my-arg": "value"}', "Tool 1 complete")
    assert status_a.children[4].value == "Agent A delegating to agent B..."

    nested_status_b = status_a.children[5]
    assert_sub_agent_status(nested_status_b, "agent_b", "task_2")
    assert nested_status_b.children[2].value == "Starting tool 2..."
    assert_popover(
        nested_status_b.children[3], "do_work_2", '{"my-arg-2": "value"}', "Tool 2 complete"
    )
    assert nested_status_b.children[4].value == "Agent B finished."
    assert len(nested_status_b.children) == 5, "Output repeating the final message is skipped"

    assert status_a.children[6].value == "Agent A finished."
    assert status_a.children[7].value == "Output:"
    assert status_a.children[8].value == "result_1"

    assert ai_message.children[2].value == "All agents have completed their tasks successfully."

    assert len(ai_message.children) == 4, (
        f"Should have 4 children: delegate to a, status for a (with nested b), final message, feedback stars - got {len(ai_message.children)}"
    )
