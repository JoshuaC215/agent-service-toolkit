...
async def main() -> None:
    ...
    recorded_audio = audiorecorder(
        "Aufnahme starten", "Aufnahme stoppen", "Pause", key="meeting_recorder"
    )
    audio_segments = []
    ...
    if uploaded_file is not None:
        ...
        audio_segments.append(
            AudioSegment.from_file(io.BytesIO(uploaded_file.getvalue()), format=ext)
        )
            ...
            if len(parsed["Breakdown"]["attendees"]) > 0:
                with col1:
                    with stylable_container("attendees", css_styles=container_css):
                        ...
                        st.markdown(
                            ", ".join(
                                [attendee["name"] for attendee in parsed["Breakdown"]["attendees"]]
                            )
                        )
                ...
                with col2:
                    with stylable_container("questions", css_styles=container_css):
                        ...
                        for question in parsed["Breakdown"]["questions"]:
                            st.markdown(
                                f"- **{question['question']}** (Raised by: {question['raised_by']}, Status: {question['status']})"
                            )
                ...
                with col2:
                    with stylable_container("insights", css_styles=container_css):
                        ...
                        for task in parsed["Breakdown"]["tasks"]:
                            st.markdown(
                                f"- **{task['description']}** (Assigned to: {task['assigned_to']}, Priority: {task['priority']})"
                            )
                ...
                with col1:
                    with stylable_container("deadlines", css_styles=container_css):
                        ...
                        for deadline in parsed["Breakdown"]["deadlines"]:
                            st.markdown(
                                f"- **{deadline['description']}** (Date: {deadline['date']})"
                            )
                ...
                with col2:
                    with stylable_container("follow_ups", css_styles=container_css):
                        ...
                        for follow_up in parsed["Breakdown"]["follow_ups"]:
                            st.markdown(
                                f"- **{follow_up['description']}** (Owner: {follow_up['owner']}, Due Date: {follow_up['due_date']})"
                            )
