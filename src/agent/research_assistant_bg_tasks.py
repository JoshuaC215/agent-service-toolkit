...
class AgentState(MessagesState, total=False):
    ...
    detailed_messages: Annotated[list[AnyMessage], add_messages]
...
async def acall_model(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    if safety_output.safety_assessment == SafetyAssessment.UNSAFE:
        ...
        return {"messages": messages, "detailed_messages": messages, "safety": safety_output}
    ...
    if state["is_last_step"] and response.tool_calls:
        ...
        return {"messages": messages, "detailed_messages": messages}
    # We return a list, because this will get added to the existing list
    return {"messages": [response], "detailed_messages": [response]}
...
async def llama_guard_input(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    return {"safety": safety_output, "detailed_messages": [start_message, end_message]}
...
async def init_state(state: AgentState, config: RunnableConfig) -> AgentState:
    return {"detailed_messages": [state["messages"][-1]]}
...
agent.add_edge("init_state", "guard_input")
...
research_assistant_bg_tasks = agent.compile(
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        result = await research_assistant_bg_tasks.ainvoke(
            inputs,
        ...
        #
        # research_assistant_bg_tasks.get_graph().draw_png("agent_diagram.png")
